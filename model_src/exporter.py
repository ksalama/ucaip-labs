...
def export_serving_model(
    ...
    features_input_signature = {
        feature_name: tf.TensorSpec(shape=(None, 1), dtype=spec.dtype, name=feature_name)
    ...
    signatures = {
        ...
        "serving_tf_example": _get_serve_tf_examples_fn(
            classifier, tft_output, raw_feature_spec).get_concrete_function(
            tf.TensorSpec(shape=[None], dtype=tf.string, name="examples")),
    }
    
    tf.saved_model.save(
        classifier, serving_model_dir, signatures=signatures
    )
